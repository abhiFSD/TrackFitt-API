...
from app.models.models import (
    ...
    ExerciseCategory as DBExerciseCategory, AITracking as DBAITracking
)
...
from app.schemas.ai_schemas import WorkoutAIRequest, WorkoutAIResponse, AITrackingResponse
...
def read_exercises(
    ...
    logger.info(f"Received request to read_exercises. Filters: category={category}, category_id={category_id}, difficulty={difficulty}, equipment={equipment}, equipment_id={equipment_id}, search={search}, exercise_uuid={exercise_uuid}, muscle_group={muscle_group}")
    
    query = db.query(DBExercise)
    
    if exercise_uuid:
        logger.info(f"Applying filter: DBExercise.exercise_id == {exercise_uuid}")
        query = query.filter(DBExercise.exercise_id == exercise_uuid)
        limit = 1
        skip = 0
    ...
    if equipment_id:
        ...
        equipment_name = db.execute(text("SELECT name FROM equipment WHERE id = :equipment_id"), 
                                   {"equipment_id": equipment_id}).fetchone()
        if equipment_name:
            equipment_name = equipment_name[0]
            # Special case for Bodyweight equipment
            if equipment_name.lower() == "bodyweight":
                # Include exercises with empty/null equipment, "None", and exercises with "Bodyweight"
                query = query.filter(
                    or_(
                        DBExercise.equipment.is_(None),
                        DBExercise.equipment == "",
                        DBExercise.equipment == "None",  # Include "None" value
                        DBExercise.equipment.ilike("%bodyweight%")
                    )
                )
            else:
                # Normal case for other equipment
                query = query.filter(DBExercise.equipment.ilike(f"%{equipment_name}%"))
    
    if muscle_group:
        # Proper JSON-based filtering for muscle groups
        try:
            # Try using PostgreSQL JSON operators if available
            muscle_group_query = f"""
                muscle_groups IS NOT NULL 
                AND muscle_groups::text <> ''
                AND EXISTS (
                    SELECT 1 FROM json_array_elements(muscle_groups::json) as mg 
                    WHERE mg->>'name' = :muscle_group
                )
            """
            query = query.filter(text(muscle_group_query).bindparams(muscle_group=muscle_group))
            logger.info(f"Applied PostgreSQL JSON filtering for muscle group: {muscle_group}")
        except Exception as e:
            # Fallback to simpler ILIKE search if JSON operators not available
            logger.warning(f"Could not use JSON filtering, falling back to ILIKE: {e}")
            
            # We need to filter in-memory for accurate results
            query = query.filter(DBExercise.muscle_groups.ilike(f"%{muscle_group}%"))
    ...
    try:
        ...
        logger.debug(f"Generated SQL Query (approx): {query.statement.compile(dialect=postgresql.dialect())}")
    except Exception as e:
        logger.debug(f"Could not compile query for logging: {e}")

    if exercise_uuid:
        exercises = query.all()
        logger.info(f"Query by UUID found {len(exercises)} exercise(s).")
    else:
        exercises = query.offset(skip).limit(limit).all()
        logger.info(f"Query with skip/limit found {len(exercises)} exercise(s).")
        
        # For muscle group filtering, if we couldn't use JSON operators, 
        # we need to filter the results in Python for accuracy
        if muscle_group and "json_array_elements" not in str(query.statement):
            logger.info("Performing in-memory filtering for exact muscle group matches")
            import json
            
            # Need to filter exercises to only include those with exact muscle group
            filtered_exercises = []
            for exercise in exercises:
                if exercise.muscle_groups and (exercise.muscle_groups.startswith('[') or exercise.muscle_groups.startswith('{')):
                    try:
                        muscle_groups_json = json.loads(exercise.muscle_groups)
                        if isinstance(muscle_groups_json, list):
                            for mg in muscle_groups_json:
                                if isinstance(mg, dict) and 'name' in mg and mg['name'] == muscle_group:
            ...
            logger.info(f"After in-memory filtering, found {len(exercises)} matching exercises")
...
# ---> ADD AI Workout Endpoint <--
@router.post("/ai/workouts", response_model=WorkoutAIResponse)
def create_ai_workout(
    request: WorkoutAIRequest,
    db: Session = Depends(get_db),
    current_user: DBUser = Depends(get_current_active_user)
):
    """
    Create a workout plan using the AI service based on user input.
    """
    # Inject the current user's ID into the request for tracking
    request.user_id = current_user.id
    
    try:
        # Call the AI service
        workout_response = ai_service.create_workout(db=db, request=request)
        return workout_response
    except ValueError as ve:
        # Handle potential errors from the AI service (e.g., API key issues, parsing errors)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(ve))
    except Exception as e:
        # Catch any other unexpected errors
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred while generating the workout.")
# ---> END AI Workout Endpoint <--

# ---> ADD AI History Endpoint <--
@router.get("/ai/history", response_model=List[AITrackingResponse])
def get_ai_history(
    db: Session = Depends(get_db),
    current_user: DBUser = Depends(get_current_active_user),
    skip: int = 0,
    limit: int = 50 # Limit the number of results per request
):
    """
    Get the AI interaction history for the current user.
    """
    # Ensure AITracking is imported from models, maybe as DBAITracking
    # (Checking imports - seems like AITracking is imported directly now)
    # Use created_at for ordering based on model definition
    history_records = db.query(DBAITracking)\
                        .filter(DBAITracking.user_id == current_user.id)\
                        .order_by(DBAITracking.created_at.desc())\
                        .offset(skip)\
                        .limit(limit)\
                        .all()
                        
    return history_records # Pydantic will automatically convert using orm_mode
# ---> END AI History Endpoint <--
...
def get_muscle_groups(
    ...
    # Query the muscle_groups table
    results = db.execute(text("""
        SELECT id, name, body_part, description
        FROM muscle_groups
        ORDER BY name
